const DUMMY_DATA = [
  {
    date_start : "Sat Sep 10 2022 11:30:00 GMT+0100",
    date_end : "Sat Sep 10 2022 13:30:00 GMT+0100",
    friends_invited : ["userId2", "userId3", "userId4"],
    friends_confirmed : ["userId2", "userId3"],
    location : "Manchester",
    description : "Event1",
    owner : "userId1"
  },
  {
    date_start : "Thu Aug 11 2022 17:30:00 GMT+0100",
    date_end : "Thu Aug 11 2022 17:30:00 GMT+0100",
    friends_invited : ["userId3", "userId4", "userId5"],
    friends_confirmed : ["userId4"],
    location : "Manchester",
    description : "Event2",
    owner : "userId2"
  },
  {
    date_start : "Sat Aug 13 2022 09:15:00 GMT+0100",
    date_end : "Sat Aug 13 2022 11:45:00 GMT+0100",
    friends_invited : ["userId1", "userId3", "userId5"],
    friends_confirmed : ["userId1"],
    location : "Liverpool",
    description : "Event3",
    owner : "userId1"
  },
  {
    date_start : "Tue Aug 09 2022 09:15:00 GMT+0100",
    date_end : "Tue Aug 09 2022 13:30:00 GMT+0100",
    friends_invited : ["userId6", "userId2", "userId5"],
    friends_confirmed : ["userId2"],
    location : "Leeds",
    description : "Event4",
    owner : "userId4"
  }
 ];
 
 module.exports = DUMMY_DATA;
 